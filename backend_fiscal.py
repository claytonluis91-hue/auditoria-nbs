...
# --- 2. MOTOR DE CÁLCULO COMPARATIVO (NOVO!) ---
def calcular_comparativo(valor, iss, pis, cofins, ibs_ref, cbs_ref, codigo_tributacao, df_regras):
    """
    Calcula o cenário ATUAL (ISS+PIS+COFINS) vs NOVO (IBS+CBS com reduções)
    """
    ...
    # 2. BUSCA REGRA DE REDUÇÃO PARA O CENÁRIO NOVO
    try:
    ...
    # 3. CÁLCULO DO CENÁRIO NOVO (Com Redução)
    # Alíquota Efetiva = Alíquota Referência * (1 - %Redução)
